#!/usr/bin/env python3
"""
run_crosschannel_live.py

Robust CrossChannel DTES runner with:
- explicit CLI config
- stage timing
- tqdm progress when available
- fallback ETA logger if tqdm is absent
- live ACO metrics
- interval filtering
- edge anchors
- autosave dtes_candidates.json
- run_metrics.json

Usage:
    python3 run_crosschannel_live.py \
      --t_min 100 --t_max 400 \
      --n0 2500 \
      --ants 60 \
      --iters 80 \
...
class LiveFractalDTESACOZeta(FractalDTESACOZeta):
    ...
    def run_aco(self) -> None:
        ...
        if tqdm is not None and self.use_tqdm:
            pbar = tqdm(iterator, desc="ACO", unit="iter")
        else:
            pbar = iterator
...
def add_edge_anchors(candidates: List[float], t_min: float, t_max: float, padding: float, step: float) -> List[float]:
    cleaned = []
    for t in candidates:
        try:
            tt = float(t)
...
def save_candidates(candidates: List[float], output: str) -> None:
    rows = [
        {"rank": i + 1, "t": float(t), "source": "run_crosschannel_live"}
...
def save_metrics(path: str, runner: LiveFractalDTESACOZeta, raw_candidates: List[float], final_candidates: List[float], args) -> None:
    data = {
        "config": vars(args),
...
def main() -> None:
    ...
    print("=== Live CrossChannel DTES runner ===")
    ...
    raw = runner.run()
    filtered = [float(t) for t in raw if args.t_min <= float(t) <= args.t_max]
    final = add_edge_anchors(filtered, args.t_min, args.t_max, args.edge_padding, args.edge_step)

    print("\n=== Candidate summary ===")
    print(f"raw candidates: {len(raw)}")
    print(f"filtered candidates: {len(filtered)}")
    print(f"edge-aware candidates: {len(final)}")

    save_candidates(final, args.output)
    save_metrics(args.metrics, runner, raw, final, args)
